...
static void white_noise(Gen *gen, Sint16 *stream, size_t stream_len)
{
  ...
  Gen genDrp = {
      .period = rand_r( &gen->seedp ) * 1.0f / RAND_MAX * gen->drpPeriod +2.0f,
      .volume = rand_r( &gen->seedp ) * gen->drpVol / RAND_MAX 
  };

  // fprintf( stdout, "v: %f p: %f \n",  genDrp.volume, genDrp.period );     
  if ( rand_r( &gen->seedp ) < RAND_MAX / 3 ) {
    for ( size_t i = rand_r( &gen->seedp ) % stream_len ; i < stream_len && genDrp.volume > 0.01f ; ++i ) {
      //fprintf( stdout, "l: %f\n", genDrp.volume  );
      stream[i] += next_sample( &genDrp );
      genDrp.volume = genDrp.volume * 0.99f;
    }
  }
}
...
int main(void)
{
    ...
    while (!quit) {
        ...
        slider(renderer, SLIDER_FREQ, 100.0f, 70.0f, 500.0f, &gen.period, 1.0f, 50.0f);
        slider(renderer, SLIDER_VOLUME, 100.0f, 140.0f, 500.0f, &gen.volume, 0.0f, 0.4f);
        slider(renderer, SLIDER_DRP_VOLUME, 100.0f, 210.0f, 500.0f, &gen.drpVol, 0.0f, 0.6f);
        slider(renderer, SLIDER_DRP_FREQ, 100.0f, 280.0f, 500.0f, &gen.drpPeriod, 1.0f, 20.0f);
